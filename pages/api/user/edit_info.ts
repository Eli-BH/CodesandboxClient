...
export default async function editUserInfo(
    ...
    try {
        ...
        const status = await pool.query(`SELECT Id, Status__c, Contact__c, Name FROM salesforce.Forms__c WHERE RecordTypeId in (SELECT id FROM salesforce.RecordType WHERE Name = 'Employment Docs') AND IsMostRecent__c = True AND Contact__c = '0031R00002HrSIJQA3'`)
